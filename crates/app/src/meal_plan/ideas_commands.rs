...
use manor_core::recipe::import::{
    extract_json_array_block_public, extract_json_block_public, ImportedRecipe, LlmClient,
};
...
async fn run_titles(client: &dyn LlmClient) -> Result<Vec<IdeaTitle>, String> {
    let first = client
        .complete(TITLES_PROMPT)
        .await
        .map_err(|_| "AI unavailable — try again later or check Settings → AI.".to_string())?;
    let parsed: Result<Vec<IdeaTitle>, _> =
        extract_json_array_block_public::<Vec<IdeaTitle>>(&first);
    ...
    let titles = match parsed {
        ...
        Err(_) => {
            ...
            let second = client.complete(&retry).await.map_err(|_| {
                "AI unavailable — try again later or check Settings → AI.".to_string()
            })?;
...
async fn run_expand(
    ...
    let prompt = format!(
        "{}Title: {}\nSummary: {}",
        EXPAND_PROMPT_PREFIX, title, blurb
    );
    let raw = client
        .complete(&prompt)
        .await
...
mod tests {
    ...
    fn stub(resps: &[&str]) -> StubLlm {
        StubLlm {
            responses: std::sync::Mutex::new(resps.iter().map(|s| s.to_string()).collect()),
        }
    }
    ...
    async fn titles_happy_path() {
        let s = stub(&[
            r#"[{"title":"A","blurb":"x"},{"title":"B","blurb":"y"},{"title":"C","blurb":"z"}]"#,
        ]);
    ...
    async fn titles_malformed_then_retry() {
        let s = stub(&["not json", r#"[{"title":"X","blurb":"x"}]"#]);
        let titles = run_titles(&s).await.unwrap();
    ...
    async fn titles_both_fail_returns_err() {
        ...
        assert!(
            err.to_lowercase().contains("invalid") || err.to_lowercase().contains("unavailable")
        );
    }
    ...
    async fn expand_happy_path() {
        let s = stub(&[
            r#"{"title":"Miso","servings":2,"prep_time_mins":5,"cook_time_mins":25,"instructions":"1. Cook.","ingredients":[{"quantity_text":null,"ingredient_name":"aubergine","note":null}]}"#,
        ]);
