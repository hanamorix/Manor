...
async fn preview_succeeds_via_jsonld() {
    ...
    Mock::given(method("GET"))
        .and(path("/tomato"))
        .respond_with(
            ResponseTemplate::new(200).set_body_raw(JSONLD_PAGE, "text/html; charset=utf-8"),
        )
        .mount(&server)
        .await;

    let url = format!("{}/tomato", server.uri());
    let preview = manor_app::recipe::importer::preview_inner(url::Url::parse(&url).unwrap(), None)
        .await
        .expect("preview succeeds");
    assert_eq!(preview.recipe_draft.title, "Tomato soup");
    assert_eq!(
        preview.import_method,
        manor_core::recipe::ImportMethod::JsonLd
    );
...
async fn fails_on_non_html() {
    ...
    Mock::given(method("GET"))
        .and(path("/pdf"))
        .respond_with(ResponseTemplate::new(200).set_body_raw(&b"%PDF-1.4"[..], "application/pdf"))
        .mount(&server)
        .await;

    let url = format!("{}/pdf", server.uri());
    let err = manor_app::recipe::importer::preview_inner(url::Url::parse(&url).unwrap(), None)
        .await
        .unwrap_err();
    assert!(
        err.to_string().to_lowercase().contains("not html")
            || err.to_string().to_lowercase().contains("html")
    );
}
...
async fn falls_back_to_llm_when_no_jsonld() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_raw(
            "<html><body>Cook onions and eat. Serves 4.</body></html>",
            "text/html",
        ))
        .mount(&server)
        .await;
    ...
    let preview =
        manor_app::recipe::importer::preview_inner(url::Url::parse(&url).unwrap(), Some(&stub))
            .await
            .unwrap();
