...
pub fn library_ranked(conn: &Connection) -> Result<Vec<ScoredRecipe>> {
    ...
    let mut scored: Vec<ScoredRecipe> = recipes
        ...
        .map(|recipe| {
            let score =
                days_since_last_cooked(conn, &recipe.id, today).unwrap_or(NEVER_COOKED_SCORE);
...
mod tests {
    ...
    fn sorted_descending_by_days_since() {
        ...
        let today = chrono::Local::now().date_naive();
        let a_date = (today - chrono::Duration::days(2))
            .format("%Y-%m-%d")
            .to_string();
        let b_date = (today - chrono::Duration::days(30))
            .format("%Y-%m-%d")
            .to_string();
        let c_date = (today - chrono::Duration::days(10))
            .format("%Y-%m-%d")
            .to_string();
    ...
    fn never_cooked_ranks_above_any_cooked() {
        ...
        let today = chrono::Local::now().date_naive();
        let d = (today - chrono::Duration::days(100))
            .format("%Y-%m-%d")
            .to_string();
