//! Approver for `complete_task` proposals.

use chrono::Utc;
use rusqlite::{params, OptionalExtension, Transaction};

use crate::assistant::proposal::{CompleteTaskArgs, Status};
use crate::assistant::task;
use crate::assistant::{Applied, ApplyError};

pub fn approve(tx: &Transaction, proposal_id: i64, diff: &str) -> Result<Applied, ApplyError> {
    let args: CompleteTaskArgs =
        serde_json::from_str(diff).map_err(|e| ApplyError::InvalidArg {
            field: "diff".into(),
            reason: e.to_string(),
        })?;

    let task_id = resolve_task(tx, args.task_id, args.title.as_deref())?;
    task::complete(tx, task_id)
        .map_err(|e| ApplyError::Internal(format!("task complete failed: {e}")))?;
    mark_applied(tx, proposal_id)?;

    Ok(Applied {
        proposal_id,
        status: Status::Applied,
        items_applied: 1,
        items_failed: 0,
        errors: vec![],
    })
}

fn resolve_task(
    tx: &Transaction,
    task_id: Option<i64>,
    title: Option<&str>,
) -> Result<i64, ApplyError> {
    if let Some(id) = task_id {
        let exists: bool = tx
            .query_row(
                "SELECT 1 FROM task WHERE id = ?1 AND completed_at IS NULL",
                [id],
                |_| Ok(true),
            )
            .optional()
            .map_err(|e| ApplyError::Internal(format!("task lookup failed: {e}")))?
            .unwrap_or(false);
        return if exists {
            Ok(id)
        } else {
            Err(ApplyError::StaleReference {
                entity: "task".into(),
                id: id.to_string(),
            })
        };
    }

    let title = title
        .map(str::trim)
        .filter(|title| !title.is_empty())
        .ok_or_else(|| ApplyError::InvalidArg {
            field: "title".into(),
            reason: "title is required when task_id is omitted".into(),
        })?;

    let matches: Vec<i64> = {
        let mut stmt = tx
            .prepare(
                "SELECT id FROM task
                 WHERE completed_at IS NULL AND lower(title) = lower(?1)
                 ORDER BY id",
            )
            .map_err(|e| ApplyError::Internal(format!("task lookup failed: {e}")))?;
        let rows = stmt
            .query_map([title], |row| row.get(0))
            .map_err(|e| ApplyError::Internal(format!("task lookup failed: {e}")))?
            .collect::<rusqlite::Result<Vec<_>>>()
            .map_err(|e| ApplyError::Internal(format!("task lookup failed: {e}")))?;
        rows
    };

    match matches.as_slice() {
        [id] => Ok(*id),
        [] => Err(ApplyError::StaleReference {
            entity: "task".into(),
            id: title.to_string(),
        }),
        _ => Err(ApplyError::Conflict(format!(
            "multiple open tasks match title '{title}'"
        ))),
    }
}

fn mark_applied(tx: &Transaction, proposal_id: i64) -> Result<(), ApplyError> {
    let now = Utc::now().timestamp();
    tx.execute(
        "UPDATE proposal SET status = ?1, applied_at = ?2, apply_errors_json = NULL WHERE id = ?3",
        params![Status::Applied.as_str(), now, proposal_id],
    )
    .map_err(|e| ApplyError::Internal(format!("proposal update failed: {e}")))?;
    Ok(())
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::assistant::db;
    use crate::assistant::proposal::{insert, NewProposal};
    use rusqlite::Connection;
    use tempfile::tempdir;

    fn fresh_conn() -> (tempfile::TempDir, Connection) {
        let dir = tempdir().unwrap();
        let conn = db::init(&dir.path().join("t.db")).unwrap();
        (dir, conn)
    }

    fn make_proposal(conn: &Connection, diff: &str) -> i64 {
        insert(
            conn,
            NewProposal {
                kind: "complete_task",
                rationale: "module-test",
                diff_json: diff,
                skill: "tasks",
            },
        )
        .unwrap()
    }

    #[test]
    fn approve_resolves_open_task_by_title_and_completes_it() {
        let (_d, mut conn) = fresh_conn();
        let task_id = task::insert(&conn, "Buy milk", None, None).unwrap();
        let diff = serde_json::json!({ "title": "buy milk" }).to_string();
        let pid = make_proposal(&conn, &diff);

        let tx = conn.transaction().unwrap();
        let applied = approve(&tx, pid, &diff).unwrap();
        tx.commit().unwrap();

        assert_eq!(applied.status, Status::Applied);
        let completed_at: Option<i64> = conn
            .query_row(
                "SELECT completed_at FROM task WHERE id = ?1",
                [task_id],
                |r| r.get(0),
            )
            .unwrap();
        assert!(completed_at.is_some());
    }

    #[test]
    fn approve_rejects_unknown_title() {
        let (_d, mut conn) = fresh_conn();
        let diff = serde_json::json!({ "title": "No such task" }).to_string();
        let pid = make_proposal(&conn, &diff);

        let tx = conn.transaction().unwrap();
        let err = approve(&tx, pid, &diff).unwrap_err();
        assert!(matches!(err, ApplyError::StaleReference { entity, .. } if entity == "task"));
    }
}
