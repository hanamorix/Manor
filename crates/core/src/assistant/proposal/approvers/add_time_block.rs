//! Approver for `add_time_block` proposals.

use chrono::Utc;
use rusqlite::{params, Transaction};

use crate::assistant::proposal::{AddTimeBlockArgs, Status};
...
fn validate(args: &AddTimeBlockArgs) -> Result<(), ApplyError> {
...
mod tests {
    ...
    #[test]
    fn approve_inserts_one_off_time_block() {
        ...
        let count: i64 = conn
            .query_row(
                "SELECT count(*) FROM time_block WHERE title = 'Deep work'",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    #[test]
    fn approve_rejects_invalid_time_range() {
        let (_d, mut conn) = fresh_conn();
        let diff = serde_json::json!({
            "title": "Deep work",
            "kind": "focus",
        ...
        assert!(matches!(err, ApplyError::InvalidArg { field, .. } if field == "end_time"));
    }
}
