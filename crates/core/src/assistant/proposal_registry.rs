...
pub fn approve(conn: &mut Connection, proposal_id: i64) -> Result<Applied, ApplyError> {
    ...
    let applied = match kind.as_str() {
        ...
        "add_to_shopping_list" => {
            approvers::add_to_shopping_list::approve(&tx, proposal_id, &diff)?
        }
        "add_recipe_quick" => approvers::add_recipe_quick::approve(&tx, proposal_id, &diff)?,
        "plan_meal" => approvers::plan_meal::approve(&tx, proposal_id, &diff)?,
